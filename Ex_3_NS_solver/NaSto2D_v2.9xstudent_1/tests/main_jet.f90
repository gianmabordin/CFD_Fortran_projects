program main
  ...
  !
  use mod_common,              only: BC_DIRICHLET, BC_NEUMANN
  !
  ...
  !
  integer :: i,j
  real(rp) :: U0, y0, half_width, delta
  real(rp) :: y
  !
  ...
  ! Setup
  ! ------------------------------------------------------------
  call CMM_InitGrid(128, 64, 5.0_rp, 1.0_rp)
  ...
  ! Boundary conditions
  ! ------------------------------------------------------------
  bc_u_west  = BC_DIRICHLET
  bc_v_west  = BC_DIRICHLET
  bc_u_east  = BC_NEUMANN
  bc_v_east  = BC_NEUMANN
  !
  bc_u_south = BC_NEUMANN
  bc_u_north = BC_NEUMANN
  bc_v_south = BC_NEUMANN
  bc_v_north = BC_NEUMANN
  !
  bc_p_west  = BC_NEUMANN
  bc_p_east  = BC_DIRICHLET
  bc_p_south = BC_DIRICHLET
  bc_p_north = BC_DIRICHLET
  !
  do j = 1, ny
     y = (real(j,rp) - 0.5_rp) * dy
     !
     bc_u_west_val(j) = 0.5_rp * U0 * &
                        (tanh((y - (y0 - half_width)) / delta) - &
                         tanh((y - (y0 + half_width)) / delta))
     !
     bc_v_west_val(j) = 0.0_rp
  end do
  !
